...
_awxctl()
{
	...
	opts="-i --info -v --version -s --start -t --stop -r --restart -h --help -c --config -l --list -e --read -a --add -d --delete"
	...
	if [[ ${cur} == -* ]]; then
		...
		COMPREPLY=( $(compgen -W "info version start stop restart help config list read add delete" -- ${cur}) )
		return 0
	fi
}
