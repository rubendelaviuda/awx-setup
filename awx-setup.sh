...
# SCRIPT THAT INSTALLS AWX AND ALL ITS DEPENDENCIES IN THE SYSTEM
# Script created by Ruben de la Viuda

function deletesetup {
	sudo rm -rf minikube-linux-amd64
	sudo rm -rf kustomize_v4.5.7_linux_amd64.tar
	sudo rm -rf kustomize_v4.5.7_linux_amd64.tar.gz
	sudo rm -rf kustomize
	sudo rm -rf kustomization.yaml
	sudo rm -rf kubectl.sha256
	sudo rm -rf kubectl
	sudo rm -rf awx-demo.yaml
...
# SCRIPT 1 THAT INSTALLS ALL AWX DEPENDENCIES
# Script created by Ruben de la Viuda
...
then
	echo ""
	echo "❌  Error checking the repositories list. Check your internet connection and/or your local repositories list"
	exit 1
fi
...
function show_status {
	# Checks if there is any "tail" process running on the console and if so, kills it
	if pgrep tail > /dev/null; then
		pkill tail
	fi

	# Function for handling escape signals
	cleanup() {
		pkill tail
		exit 0
	}
	...
	# If it is not, it will display a notification
	...
	# If it is running
		...
		# Checks if there is any process modifying the container
		if ! grep -q "Done!" /var/lib/awx/awx-status; then
		# Displays the status of the process
			tail -f /var/lib/awx/awx-status &
			# Checks once per second if the modification of the container has finished
			while true
			do
				# If the modification has finished, then kills the tail process and breaks out of the loop
					if grep -q "Done!" "/var/lib/awx/awx-status"; then
					pkill tail
					break
				else
		...
		# Checks again if the container is stopped
		...
		if minikube status 2> /dev/null | grep -q "Stopped"; then
			...
			echo -e "❗  AWX container is currently stopped"
			echo ""
		# If not, display the AWX credentials
		else
			echo ""
			printf '\033[1m💡  AWX INFORMATION  💡\n-----------------------\n\033[0m'
			echo ""
...
function start_awx {
	# Script title
	echo "" | tee /var/lib/awx/awx-status
	printf '\033[1m🚀  STARTING AWX CONTAINER  🚀\n------------------------------\n\033[0m' | tee -a /var/lib/awx/awx-status
...
function stop_awx {
	# Script title
	echo "" | tee /var/lib/awx/awx-status
	printf '\033[1m🚩  STOPPING AWX CONTAINER  🚩\n------------------------------\n\033[0m' | tee -a /var/lib/awx/awx-status
	echo "" | tee -a /var/lib/awx/awx-status
...
function restart_awx {
	# Script title
	echo "" | tee /var/lib/awx/awx-status
	printf '\033[1m🔁  RESTARTING AWX CONTAINER  🔁\n------------------------------\033[0m\n\n' | tee -a /var/lib/awx/awx-status

	# Display message indicating that the container will be stopped
	printf '\033[1m🚩  Stopping the AWX container\033[0m\n'  | tee -a /var/lib/awx/awx-status

	# Stops the container
	minikube stop | tee -a /var/lib/awx/awx-status && printf '✅  AWX stopped succesfully\n\n' | tee -a /var/lib/awx/awx-status

	# Display message indicating that the container will be started
...
# Changes the permissions of the file /bin/awx-help
...
_awxctl()
{
	...
	opts="-i --info -s --start -t --stop -r --restart -h --help"
	...
	if [[ ${cur} == -* ]]; then
		...
		COMPREPLY=( $(compgen -W "info start stop restart help" -- ${cur}) )
		return 0
	fi
}
...
# SCRIPT THAT STARTS THE AWX CONTAINER ON DOCKER IN MINIKUBE IN THE BACKGROUND
# Script made by Ruben de la Viuda
...
# SYSTEMD SERVICE CONFIGURATION FOR AUTOMATIC START OF AWX CONTAINER ON DOCKER IN MINIKUBE
# Configuration made by Ruben de la Viuda
...
if ! id -nG | grep -qw docker; then
	echo ""
	echo "❌  The user must be added to the 'docker' group to run this script"
	deletesetup
fi
